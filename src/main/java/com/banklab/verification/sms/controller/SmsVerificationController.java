...
public class SmsVerificationController {
    ...
    public ResponseEntity<StatusResponse> sendCode(@RequestBody PhoneSendDTO request){
        String phoneNum = request.getPhone().replace("-", "");
        smsService.sendVerificationCode(phoneNum);
        return ResponseEntity.ok(new StatusResponse(true, "인증번호 발송 완료"));
    }

    @PostMapping("/verify")
    @ApiOperation(value = "인증번호 검증")
    public ResponseEntity<StatusResponse> verifyCode(@RequestBody PhoneVerifyDTO request){
        String phoneNum = request.getPhone().replace("-", "");
